...
public class RaportImpl implements Raport {
	...
	public List pobierzDaneDoRaportu(String rok) {
		...
		String hqlPlanuPracy = "SELECT MONTH(Data_platnosci) as  Miesiac,"
				+ " COUNT(*) as Ilosc,SUM(kwota) as  Przychod FROM Platnosc" + " WHERE YEAR(Data_platnosci)=2017"
	...
	public double[] pobierzKosztyZamowien() {
		...
		String hqlPlanuPracy = "SELECT MONTH(Data_zamowienia) as Miesiac, Sum(Produkt.Cena*Produkt_Zamowienia.Ilosc) as SumaKosztow"
				+ " FROM Zamowienia,Produkt,Produkt_Zamowienia" + " WHERE YEAR(Data_zamowienia)=2017"
				+ " AND Produkt.ID=Produkt_Zamowienia.ProduktID" + " AND Produkt_Zamowienia.ZamowieniaID=Zamowienia.ID"
		...
		while (it.hasNext()) {
			...
			for (int i = 0; i <= 11; i++) {
				...
				} else if ((Integer.parseInt(object[0].toString()) != i
	...
	public List pobierzLiczbeWizytWedlugMiesiaca(String rok) {
		...
		String query = "SELECT MONTH(Data) as  Miesiac," + " COUNT(*) as LiczbaWizyt "
				+ "FROM Wizyta WHERE YEAR(Data)=2017 " + "GROUP BY MONTH(Data)";
		...
		while (it.hasNext()) {
			...
			for (int i = 0; i <= 11; i++) {
				...
				if (Integer.parseInt(object[0].toString()) == i) {
					...
					// System.err.println(Double.parseDouble(object[1].toString())
					// + "obieg petli :" + i);
	...
	public List poberzLiczbeZamowienWedlugMiesiaca(String rok) {
		...
		String query = "SELECT MONTH(Data_zamowienia) as  Miesiac," + " COUNT(*) as LiczbaZamowien "
				+ "FROM Zamowienia WHERE YEAR(Data_zamowienia)=2017 " + "GROUP BY MONTH(Data_zamowienia)";
		SQLQuery databaseQuery = session.createSQLQuery(query);
