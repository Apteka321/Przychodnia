...
public class ZamowienieImpl implements Zamowienie {
	...
	public Pielegniarka pobierzPielegnierkeWedlugID(int identyfikatorPielegniarki) {
		...
		// Orzygotowanie zapytania pobieraj¹cego dane pielegniarki o wskazanym
		// id
	...
	public String utworzZamowienie(ObservableList<ProduktZamowienia> listaProduktowZamowienia,
		...
		// W ka¿dym cyklu pêtli pobierany jest obiekt Pozycji Zamówienia
		// zapiszEdytowaneZamowienie listy Zamowieñ przygotowanych
		// przez u¿ytkownika
		for (NoweZamowienieController.ProduktZamowienia produktZamowienia : listaProduktowZamowienia) {
			...
			Sala sala = produktZamowienia.getNrSali();
			Produkt produkt = new Produkt();
			produkt.setCena(new BigDecimal(0));
			// Pobranie nazwy produktu zapiszEdytowaneZamowienie listy lisyt
			// przedmiotow zamówienia
			...
			noweZamowienie.setGodzina(new Time(new Date().getTime()));
	...
	public List<Produkt_Zamowienia> pobierzProduktZamowienia(Integer idZamowienia) {
		...
		Query listaProduktowZamowienia = session
				.createQuery("from Produkt_Zamowienia where ZamowieniaID=" + idZamowienia + "order by ProduktID");
	...
	public String zapiszEdytowaneZamowienie(
			ObservableList<ZatwierdzenieZamowieniaController.ZatwierdzenieZamowienia> listaProduktowDoWyswietlenia) {
		Session session = HibernateUtil.getSessionFactory().openSession();
