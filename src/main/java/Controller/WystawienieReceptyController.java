...
public class WystawienieReceptyController {
	...
	// zalogowany lekarz
	Lekarz lekarz = new KontaRepositoryImpl().getListaLekarzy().get(0);
	// wybrana wizyta
	Wizyta wizyta = new WizytaRepositoryImpl().getListaWizyt().get(0);
	// wybrany pacjent
	Pacjent pacjentOsoba = new KontaRepositoryImpl().getPacjentowByPESEL(wizyta.getPESEL_pacjenta().toString());
	...
	void initialize() {
		...
		nazwaLekuColumn.setCellValueFactory(new PropertyValueFactory<>("nazwa"));
		producentColumn.setCellValueFactory(new PropertyValueFactory<>("producent"));
		refundacjaColumn.setCellValueFactory(new PropertyValueFactory<>("Procent_refundacji"));
		dawkowanieColumn.setCellValueFactory(new PropertyValueFactory<>("Dawkowanie"));
		usunColumn.setCellValueFactory(new PropertyValueFactory<>("usun"));

		tabela.setItems(rekordyWystawionychLekow);

		System.out.println("Zakonczono inicjalizacje!");
	}
	...
	String generujRecepteHTML() {
		...
		List<String> kodHTML = new ArrayList<>();
